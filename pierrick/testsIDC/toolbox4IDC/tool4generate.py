...
#=========================================================================
def generator(xsensors_m, settingfile):
    ...
    #=====================================================================
    # Inputs:
    #    - xsensors_m array M x 3 sensor coordinates in meter
    #    - settingfile is a text file which consists of the 
    #          parameter affectation useful for the function
    #          synthetizer (see above)
    # outputs:
    #     - signals : NxM array but 0 is generation impossible.
    #     - groundtruth (see below)
    #=====================================================================

    """
    altext = loadtxt(settingfile, comments='#', dtype='str',
                  delimiter='\n', converters=None, skiprows=0, 
                  usecols=None, unpack=False, ndmin=0)
                  
    laltext = len(altext)
    for iell in range(laltext):
        exec(str(altext[iell]))
        
    SON_frequencyband_Hz = array([SON_frequencyband_Hz_inf, SON_frequencyband_Hz_sup])
    ...
    SOIdurationrange_sec        = array([SOIdurationrange_sec_inf, SOIdurationrange_sec_sup]);
    SOIfrequencywidth_Hz        = array([SOIfrequencywidth_Hz_inf, SOIfrequencywidth_Hz_sup]);
    SOISNRrange_dB              = array([SOISNRrange_dB_inf, SOISNRrange_dB_sup]);
    
    SOIazimuthrange_deg          = array([SOIazimuthrange_deg_inf, SOIazimuthrange_deg_sup]);
    SOIelevationrange_deg        = array([SOIelevationrange_deg_inf, SOIelevationrange_deg_sup]);
    SOIvelocityrange_mps         = array([SOIvelocityrange_mps_inf, SOIvelocityrange_mps_sup]);
...
#    return SOI, LOC, SON, hugeNoise, failure, emergent
#    SOI, LOC, SON, hugeNoise, failure, emergent = readsetting(settingfile)
    signals, groundtruth  = synthetizer(xsensors_m, SOI, LOC, \
                SON, hugeNoise, failure, emergent)
    
    return signals, groundtruth
    
#=========================================================================
def synthetizer(xsensors_m, SOI, LOC, SON, 
                          hugeNoise, failure, emergent):                             
    """
    #=====================================================================
    #
    # SYNOPSIS:  (observationswithnoise, groundtruth) = 
    #    synthetizer(xsensors_m, SOI, LOC, SON, hugeNoise, failure, emergent)
    #
    #=====================================================================
    # Inputs:
    #     xsensors_m = array M x 3 sensor coordinates in meter
    ...
    #         M: number of sensors
    #     groundtruth: structure sequence, one structure for each detection
    #     SOI = struct(name = 'soiinfo', 
    #             soiflag_real = SOIflag_real,
    #             soidatabase = SOI_database,
    #             noiseflag_real = NOISEflag_real,
    ...
    #     phasedistorsion = 
    """
    flag_too_many_detections    = False;
    Fs_Hz                       = SOI.Fs_Hz;
...
#    
    for ik in range(nb_events):
        ...
        groundtruth.append(onedetection)
    
    if flag_too_many_detections:
        observationswithnoise = nan
    
    return (observationswithnoise, groundtruth)
