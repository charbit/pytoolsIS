# -*- coding: utf-8 -*-
"""
Created on Sun Mar 13 13:03:35 2016
...
#!/usr/bin/env python
"""
UTMZone, UTMEasting, UTMNorthing=ll2utm(Lat, Long, ReferenceEllipsoid=23)
lon,lat=utm2ll(easting, northing, zone, ReferenceEllipsoid=23):
...
def ll2utm(Long, Lat, ReferenceEllipsoid=23):
    """
    (UTMZone, UTMEasting, UTMNorthing)=ll2utm(Long, Lat, ReferenceEllipsoid)
...
def utm2ll(easting, northing, zone, ReferenceEllipsoid=23):
    """
    lon,lat=utm2ll(easting, northing, zone, ReferenceEllipsoid=23)
...
#    (z, e, n) = ll2utm(40, -88, ReferenceEllipsoid=23)
#    print z, e, n
#    print utm2ll(e, n, z)


#===============================================================
def extractstationlocations(XLSfile,numselect, ReferenceEllipsoid=23):
    """
    extract information from the XLS file
    synopsis
         extractstationlocations(XLSfile,numselect, ReferenceEllipsoid=23)
...
#    import openpyxl
#    wb = openpyxl.load_workbook('../xsensor/allISs.xls')
    ...
    for rownum in range(sh.nrows):
        if (sh.cell_value(rownum,0)[0:3]==numselect):
            sensor.append(sh.cell_value(rownum,0))
            lat[cp]=float(sh.cell_value(rownum,colLat))
            long[cp]=float(sh.cell_value(rownum,colLong))
            elev[cp]=float(sh.cell_value(rownum,colElev))
            cp=cp+1
    nbsensors = cp;
    lat=lat[0:nbsensors]
    long=long[0:nbsensors]   
    elev=elev[0:nbsensors]
    sensor=sensor[0:nbsensors]       
    
    station = []

    for im in range(nbsensors):
            [ex, x,y] = ll2utm (long[im], lat[im], ReferenceEllipsoid);
            geoloc_im = struct(latitude_deg=lat[im], longitude_deg=long[im], \
                Z_km = elev[im], \
                X_km = x/1000.0, \
                Y_km = y/1000.0)
            st_aux = struct(sensorname=sensor[im], geolocs=geoloc_im)
            station.append(st_aux)
